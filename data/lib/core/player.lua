...
do
	...
	function Player.getTaskPoints(self)
		local value = self:getStorageValue(200001)
		return (value and value > 0) and value or 0
	end

	function Player.addTaskPoints(self, amount)
		self:setStorageValue(200001, self:getTaskPoints() + amount)
	end

	function Player.removeTaskPoints(self, amount)
		local current = self:getTaskPoints()
		if current >= amount then
			self:setStorageValue(200001, current - amount)
			return true
		end
		return false
	end
