...
void ProtocolGame::parsePacket(NetworkMessage& msg)
{
	...
	switch (recvbyte) {
		...
		case 0xF9:
			...
			break;
		case 0xFA: /* client open store */
			sendCoinBalance(static_cast<uint32_t>(IOLoginData::getTibiaCoins(player->getAccount())));
			break;
...
void ProtocolGame::sendCoinBalance(uint32_t coins)
{
	NetworkMessage msg;
	msg.addByte(0xDF);
	msg.addByte(1); // update = true
	msg.add<uint32_t>(coins);
	msg.add<uint32_t>(0); // transferableCoins
	writeToOutputBuffer(msg);
}
